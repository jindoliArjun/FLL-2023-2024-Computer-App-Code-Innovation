                ...
                for i in range(6):
                    ...
                    if (mouse_x > button_x - BUTTON_RADIUS and mouse_x < button_x + BUTTON_WIDTH + BUTTON_RADIUS and
                        ...
                        if page == 4:
                            ...
                            nutrient_text = pygame_gui.windows.UIMessageWindow(rect=pygame.Rect((SCREEN_WIDTH // 2 - 200, SCREEN_HEIGHT // 2 - 100), (400, 200)), html_message="<b>Soil Nutrient Test</b>", manager=manager, window_title="Soil Nutrient Test")
                            nutrient_table = pygame_gui.elements.UITable(relative_rect=pygame.Rect((0, 50), (400, 150)), manager=manager, container=nutrient_text, column_headings=["Nutrient", "Percentage"])
                        ...
                        if page == 5:
                            sunlight_scan = random.choice(sunlight_levels)
                            sunlight_text = pygame_gui.windows.UIMessageWindow(rect=pygame.Rect((SCREEN_WIDTH // 2 - 200, SCREEN_HEIGHT // 2 - 100), (400, 200)), html_message=f"<b>Sunlight Test</b>: {sunlight_scan}", manager=manager, window_title="Sunlight Test")
                            scan_button = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 - 200), (200, 50)), text="SCAN", manager=manager)
                            scan_button.disable()
                            pygame.time.set_timer(pygame.USEREVENT, 5000) # wait for 5 seconds
                            for event in pygame.event.get():
                                if event.type == pygame.USEREVENT:
                                    scan_button.enable()
                                    sunlight_text.kill() # close the text box
                                    sunlight_scan = random.choice(sunlight_levels)
                                    sunlight_text = pygame_gui.windows.UIMessageWindow(rect=pygame.Rect((SCREEN_WIDTH // 2 - 200, SCREEN_HEIGHT // 2 - 100), (400, 200)), html_message=f"<b>Sunlight Test</b>: {sunlight_scan}", manager=manager, window_title="Sunlight Test")
                                    scan_button.disable()
                                    pygame.time.set_timer(pygame.USEREVENT, 5000) # wait for 5 seconds
